...
Try {
	...
	[string]$appVersion = 'R2017a'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.3'
	[string]$appScriptDate = '06/06/2017'
	...
	If ($deploymentType -ine 'Uninstall') {
	  ...
	  If (-not (Test-Path "$envProgramFiles\MATLAB\R2017a")) {
				New-Folder -Path "$envProgramFiles\MATLAB\R2017a"
		}

		Copy-item -Path "$dirSupportFiles\license.dat" -Destination "$envProgramFiles\MATLAB\R2017a\license.dat"
		...
		Remove-File -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\MATLAB R2017a\Activate MATLAB R2017a.lnk"
		Remove-File -Path "$envProgramData\Microsoft\Windows\Start Menu\Programs\MATLAB R2017a\Deactivate MATLAB R2017a.lnk"
		Execute-Process -Path "$envSystem32Directory\netsh.exe" -Parameters "advfirewall firewall add rule name=`"MATLAB R2017a`" dir=in action=allow program=`"C:\program files\matlab\r2017a\bin\win64\matlab.exe`" description=`"MATLAB R2017a`" enable=yes profile=any protocol=tcp edge=deferuser" -WindowStyle "Hidden"
		Execute-Process -Path "$envSystem32Directory\netsh.exe" -Parameters "advfirewall firewall add rule name=`"MATLAB R2017a`" dir=in action=allow program=`"C:\program files\matlab\r2017a\bin\win64\matlab.exe`" description=`"MATLAB R2017a`" enable=yes profile=any protocol=udp edge=deferuser" -WindowStyle "Hidden"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAkz4uQIIPARNgu
# xeENMFa7xi3pIEgjvkeYKsB6pkj+0aCCD4cwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# gAAwGQYJKoZIhvcNAQkDMQwGCisGAQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIPpe
# 3Z/OVBuwFcv1FbjbyfugrkD1whKJFBojtQveG1pLMA0GCSqGSIb3DQEBAQUABIIB
# ADoTZEBEQmSUNlGlaCDN1IqnSoVNwCOtAyO3+bvXb34UVeXvAksRoQIT8qceJpXt
# Ds+Cf3gMAc1Snt2i8JOWulj+/BMvB+QFhs7TNimeNBnaGgnaUAWaLHgPuXy2yE4a
# pku9Vo8t17/rGBpdmqU/jS9WkG/YyceTxvkeAujIKhHaXBEizmYaDYFsxnbAUyAr
# aLmWI4ig2aJGq8Liv50/0Vzf08MrtgnPHZOp/5yX/ImTbyQj2b7WYBLRDk48RaM6
# nnZxSckq2zRxLhho0BqxA8se2RK+gLpD6ZU4ia4PCQPNjqwT9mw/cGghhfWHqcgJ
# Lt4tvsIA7ZiVIDj6ilhU6NihggKiMIICngYJKoZIhvcNAQkGMYICjzCCAosCAQEw
...
# ATAcBgkqhkiG9w0BCQUxDxcNMTcwMTA0MTgwNTI4WjAjBgkqhkiG9w0BCQQxFgQU
# uKWV//2GsiwWOYQvw4RygDfy3skwgZ0GCyqGSIb3DQEJEAIMMYGNMIGKMIGHMIGE
...
# AQUABIIBAE5pWVqly7dF0xYHn/7RV67yJ9xobVPDndeov7ykjs48e8HlIYs0MhW5
# X0YcHk+R0IkX8u2GYi06310vczoeCcK04D5VEhVJDv/03uM7XLejqRKb4PBDScWy
# c2vg5nnDYG+yKv6O019sWbnACNzLKBZUCOb0xMzh56jVEmBtdWsVs1g6AeefeHlL
# EsLwwXSD38oZnk+Inj2bXUBkgw3xmNzPQ06ae36HoJRjYO2GFw4ARdcOLMLDsnya
# dz8ztQACrEeYki0QB02s+gzOi+McWgoJlh7YABVGL/g9nV87VtFYvodsKk6yLHTV
# gsRvZXpCUngFSle1eG1RTJoHtGTWxTA=
